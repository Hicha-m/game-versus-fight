...
static int test_arena_get_tile_returns_error_for_null(void) {
	GameError err = GAME_OK;
	TileType tile = arena_get_tile(NULL, 0U, 0U, &err);

	TEST_ASSERT_EQ_INT(GAME_ERROR_OUT_OF_BOUNDS, err);
	TEST_ASSERT_EQ_INT(TILE_EMPTY, tile);
	return 0;
}

static int test_arena_find_spawn_returns_false_for_null(void) {
	Vec2i spawn = {-1, -1};
...
int run_arena_tests(void) {
	TestCase cases[] = {
		...
		{"get_tile_returns_error_for_null", test_arena_get_tile_returns_error_for_null},
		{"find_spawn_returns_false_for_null", test_arena_find_spawn_returns_false_for_null},
	};
