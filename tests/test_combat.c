...
static int test_combat_step_sword_hit_ends_round(void) {
	Arena arena = {0};
	CombatState combat = {0};
	PlayerCommand p1 = {0};
	PlayerCommand p2 = {0};
	...
	TEST_ASSERT_TRUE(combat.round_over);
	TEST_ASSERT_EQ_INT(0, combat.winner_index);
	TEST_ASSERT_TRUE(!combat.fighters[1].state.alive);
	return 0;
}
...
int run_combat_tests(void) {
	TestCase cases[] = {
		...
		{"step_sword_hit_ends_round", test_combat_step_sword_hit_ends_round},
